...
| 2026-07-23 | Release v1.0.5 (Returns Portal, WhatsApp Gateway, GPS Geocoding, Spin Wheel) | Added `order_returns` table, `returnController.js` (/api/returns), `whatsapp.js` notification gateway, GPS reverse geocoding, 360° product spinner, and Lucky Wheel rewards. |
| 2026-07-23 | Syntax error in `db.js` active_sessions migration block | Fixed missing catch block closing brace which caused server initialization crash. |
