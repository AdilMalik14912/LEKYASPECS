...
export default function Account() {
  ...
  if (!user) {
    return (
      ...
      <div className="bg-premium-black min-h-screen py-16 sm:py-24 flex items-center justify-center">
        <div className="max-w-md w-full mx-4 bg-white border border-premium-border rounded p-6 sm:p-10 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button
              type="button"
              onClick={() => { setSocialModalProvider('google'); setSocialError(''); setSocialModalOpen(true); }}
            ...
            >
              <svg className="w-4 h-4 shrink-0 transition-transform group-hover:scale-110" viewBox="0 0 24 24">
                <path fill="#EA4335" d="M12.24 10.285V14.4h6.887c-.648 2.41-2.519 4.114-5.137 4.114-3.478 0-6.3-2.823-6.3-6.3 0-3.478 2.822-6.3 6.3-6.3 1.63 0 3.106.625 4.225 1.637l3.136-3.136C19.123 2.502 15.86 1 12.24 1 6.033 1 1 6.033 1 12.24s5.033 11.24 11.24 11.24c5.895 0 10.865-4.224 10.865-11.24 0-.668-.057-1.314-.165-1.955H12.24z" />
              </svg>
            ...
            <button
              type="button"
              onClick={() => { setSocialModalProvider('facebook'); setSocialError(''); setSocialModalOpen(true); }}
      ...
      {/* Social Auth Account Selector Modal — shown to unauthenticated users */}
      {socialModalOpen && (
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center p-4"
          style={{ background: 'rgba(0,0,0,0.85)' }}
          onClick={(e) => { if (e.target === e.currentTarget) setSocialModalOpen(false); }}
        >
          <div className="bg-[#12021c] border border-[#FAAE62]/40 rounded-2xl p-6 sm:p-8 max-w-sm w-full shadow-2xl relative">
            <button
              type="button"
              onClick={() => setSocialModalOpen(false)}
              className="absolute top-3 right-3 text-gray-400 hover:text-white p-1.5 rounded-full hover:bg-white/10 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex flex-col items-center text-center mb-5">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Enter your account details to continue to <strong className="text-white">Lekya Specs</strong>
              </p>
            </div>
            ...
            <form onSubmit={handleSocialSubmit} className="space-y-3">
              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-wider text-gray-300 mb-1">Full Name</label>
                <input
                  type="text"
                  required
                  value={socialName}
                  ...
                  className="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#FAAE62] transition-colors"
                />
              </div>
              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-wider text-gray-300 mb-1">
                  {socialModalProvider === 'google' ? 'Gmail Address' : 'Facebook Email'}
                </label>
                <input
                  type="email"
                  required
                  ...
                  placeholder={socialModalProvider === 'google' ? 'yourname@gmail.com' : 'yourname@example.com'}
                  className="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-[#FAAE62] transition-colors"
                />
              </div>
              <button
                ...
                className={`w-full mt-2 py-3.5 rounded-lg font-bold text-sm text-white transition-all shadow-lg flex items-center justify-center gap-2 ${
                  socialModalProvider === 'google' ? 'bg-[#1a73e8] hover:bg-[#1557b0]' : 'bg-[#1877F2] hover:bg-[#1464cc]'
                } disabled:opacity-60 disabled:cursor-not-allowed`}
              >
                {socialLoading ? (
                  <><Loader2 className="w-4 h-4 animate-spin" /> Signing in...</>
                ) : (
                  <>Continue as {socialName ? socialName.split(' ')[0] : 'Me'}</>
                )}
              </button>
            </form>

            <p className="mt-4 text-[10px] text-gray-500 text-center">
              By continuing, you agree to Lekya Specs'
              {' '}<a href="/privacy" className="text-[#FAAE62] hover:underline">Privacy Policy</a>.
            </p>
          </div>
        </div>
      )}
