...
import { Gift, X, Sparkles, Check, Copy, Zap } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-md bg-[#1A0024] border border-[#FAAE62]/40 rounded-3xl p-6 shadow-2xl text-center overflow-hidden">
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FAAE62]/10 border border-[#FAAE62]/30 text-[#FAAE62] text-xs font-bold uppercase tracking-wider mb-2">
          <Sparkles className="w-3.5 h-3.5" /> Spin & Win Rewards
        </div>

        <h3 className="text-2xl font-serif font-bold text-white mb-1">Lucky Reward Wheel</h3>
        <p className="text-xs text-[#9B7EA8] mb-6">Spin the wheel to unlock exclusive discounts & luxury perks!</p>

        {/* Wheel Container */}
        <div className="relative mx-auto w-64 h-64 mb-6">
          {/* Top Pointer Indicator */}
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10 w-0 h-0 border-l-8 border-r-8 border-t-[16px] border-l-transparent border-r-transparent border-t-[#FAAE62] filter drop-shadow-md"></div>

          <canvas
            ref={canvasRef}
            width={256}
            height={256}
            className="w-full h-full rounded-full shadow-2xl cursor-pointer"
            onClick={handleSpin}
          />
        </div>

        {/* Action Button / Reward Banner */}
        {wonReward ? (
          <div className="bg-[#2A0440] border border-[#FAAE62]/50 rounded-2xl p-4 animate-scaleUp">
            ...
            <div className="flex items-center justify-between bg-[#0D0016] border border-white/10 rounded-xl p-2 px-3">
              ...
              <button
                onClick={copyCode}
                className="flex items-center gap-1 text-xs bg-[#FAAE62] text-[#0D0016] font-bold px-3 py-1.5 rounded-lg hover:bg-[#FCC48A] transition-colors"
              >
          ...
          <button
            onClick={handleSpin}
            disabled={spinning}
            className="w-full bg-gradient-to-r from-[#D4893F] to-[#FAAE62] hover:scale-105 active:scale-95 text-[#0D0016] font-black text-sm uppercase tracking-widest py-3 rounded-xl shadow-lg shadow-[#FAAE62]/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
