...
// Generates a random 5-character alphanumeric code, uppercase only (no ambiguous chars)
...
function generateSvgCaptcha(code) {
  ...
  // Draw noise circles in the background
  for (let i = 0; i < 30; i++) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const op = (Math.random() * 0.35 + 0.15).toFixed(2);
    svg += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="rgba(197, 160, 40, ${op})" stroke-width="${(Math.random() * 1.5 + 0.8).toFixed(1)}" />`;
  }

  // Render individual letters with rotation, offset, and color variation
  const colors = ['#C5A028', '#e8c547', '#f5f5f5', '#aaa', '#d4870c', '#fff9c4'];
  const charWidth = Math.floor(width / (code.length + 1));

  for (let i = 0; i < code.length; i++) {
    const char = code.charAt(i);
    const color = colors[Math.floor(Math.random() * colors.length)];
    const fontSize = Math.floor(Math.random() * 6) + 24; // 24px to 29px
    const x = Math.floor(charWidth * (i + 0.6)) + Math.floor(Math.random() * 6) - 3;
    const y = Math.floor(height / 2) + 9 + Math.floor(Math.random() * 8) - 4;
    const angle = Math.floor(Math.random() * 30) - 15; // Rotate between -15 and +15 deg
